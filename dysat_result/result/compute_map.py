...
if __name__ == '__main__':
    ...
    for data in datas:
        if timesteps == 1:
            embedding_path = os.path.join(data, 'embedding')
            embedding_file_list = os.listdir(embedding_path)
        else:
            embedding_path = os.path.join(data, 'multi_embedding')
        ...
        if timesteps == 1:
            for i in range(len(embedding_file_list)):
                MAP_list = []
                AUC_list = []
                pd_frame = pd.read_csv(os.path.join(embedding_path, embedding_file_list[i]))
                embedding_matrix = pd_frame.values
